...
flowchart TD
    ...
    ApprovalGate --> ApprovalBroker[ApprovalBroker]
...
sequenceDiagram
    ...
    R->>B: ThoughtEvent
    R->>B: ToolCallEvent
    R->>G: run(GuardedToolRequest)
    G->>AP: request_for(ToolCall)
    alt no approval required
        G-->>R: ToolExecutionEvent
        R->>B: ToolExecutionEvent
        G->>E: execute_allowed(ToolCall)
        E->>T: arun(args)
        T-->>E: ToolResult
        E-->>G: ExecutionResult
...
- 将 `ToolCall` 交给 `ApprovalGate`
- 记录 `Step`
- 压缩旧 step 到 memory
- 发出 runtime event
- 在 `final_answer` 后生成 `AgentResult`

Runtime 不直接理解每个工具的业务逻辑，不直接调用 LLM API，也不关心 policy / approval 的细节。它只依赖 `Agent` 和 `ApprovalGate` 两个协作对象。
...
- 需要审批时生成 `ApprovalRequestEvent`。
- 调用 `ApprovalBroker` 获取 approve / reject / edit 决策。
- 发出 `ApprovalDecisionEvent`。
- reject 时返回 rejected `GuardedToolOutcome`，不调用工具。
- approve 时执行原始 `ToolCall`。
- edit 时执行修改后的 `ToolCall`。
- 发出 `ToolExecutionEvent` 并调用 `ToolExecutor.execute_allowed()` 执行已放行的调用。

这样 Runtime 不需要知道审批规则、用户交互方式或拒绝结果如何构造。
