...
LLM 只负责生成结构化 `Decision`，不直接执行工具。所有工具调用必须经过 Runtime 的 step loop，再由 `Executor` 解析、校验和执行。
...
- 其他 shell 命令返回 `require_approval`，Runtime 不静默执行。

这不是完整审批系统，但它建立了一个重要边界：高风险 generic tool 不会因为模型生成了调用就自动运行。后续真正的审批流应该扩展 `PolicyDecision` 和 `Executor`，而不是绕过 policy。
