...
当前策略下，`shell_exec` 默认需要 CLI 审批；如果已有专用工具能满足需求，例如 `cat`、`ls`、`mkdir`、`rm`，Policy 会拒绝并建议改用对应专用工具。即使审批通过，`shell_exec` 也会限制 `cwd` 在 workspace 内、使用 `shell=False` 执行解析后的 argv、过滤环境变量、截断 stdout/stderr，并把非零退出码标记为失败。
...
所有工具调用都必须经过 `ToolExecutor`，再由 `PolicyEngine` 判断是否允许。当前版本已经把 `shell_exec` 作为 high risk generic tool，并对 `require_approval` 接入 CLI 用户确认；后续会加入更强的进程级沙箱和更细的权限级别。
