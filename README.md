...
当前版本已经实现了单 Agent 的 step loop、Tool 注册与解析、执行前 Policy 校验、事件流式输出、JSONL trace、CLI 交互和基础测试。后续会继续补强权限审批、工作目录沙箱、端到端测试和更多 LLM provider。
...
取舍：当前高风险 shell 调用会被阻断或要求审批，不会自动恢复执行。这让行为更保守，但避免模型生成 shell 命令后被静默执行。
...
- 完整 approval flow：让 `require_approval` 可以接入用户确认、审批记录和恢复执行。
...
1. 完善 approval flow，让 `require_approval` 可以恢复执行。
2. 强化高风险工具隔离和审计。
3. 增加可恢复 state persistence。
4. 扩展多 provider LLM 支持和 fallback 策略。
5. 补 CLI 端到端测试和上线前回归场景。
